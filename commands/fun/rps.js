...
module.exports = {
  ...
  run: async(client,msg,args) => {
		const embed = new MessageEmbed().setDescription('Add a reaction to one of these emojis to play the game!');

		const message = await msg.channel.send(embed);
		const reacted = await client.promptMessage(message, msg.author, 30, chooseArr);
